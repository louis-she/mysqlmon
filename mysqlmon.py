...
def slave_change_master(slavedb, master_host, master_port, master_logfile, master_logpos, master_user, master_password):
    cursor = slavedb.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("stop slave;")
    cursor.execute("change master to master_host='{host}', master_user='{user}', master_password='{password}', master_port={port}, master_log_file='{logfile}', master_log_pos={logpos};".format(host=master_host, user=master_user, password=master_password, port=master_port, logfile=master_logfile, logpos=master_logpos))
...
def master_routine(master):
    ...
    if not db:
        do(change_master, {"slaves": SLAVES})
